...
public struct SearchQAProvider: SearchQAProviding {
    ...
    public func generate(request: SearchQAProviderRequest) async throws -> SearchQAProviderResponse {
        ...
        let rawResponse = try await ProviderExecutionSupport.executeWithRetry(policy: config.retry_policy) { _ in
            let clientRequest = ProviderClientRequest(task: .qa_with_citations, input_json: inputJSON)
            do {
        ...
        return try ProviderExecutionSupport.decodeValidated(
            ...
            validate: SearchQASchemaValidator.validate
        )
    }
}
