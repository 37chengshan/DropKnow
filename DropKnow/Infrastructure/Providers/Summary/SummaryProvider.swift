...
public struct SummaryProvider: SummaryProviding {
    ...
    public func generate(request: SummaryProviderRequest) async throws -> SummaryProviderResponse {
        ...
        let rawResponse = try await ProviderExecutionSupport.executeWithRetry(policy: config.retry_policy) { _ in
            let clientRequest = ProviderClientRequest(task: .summary, input_json: inputJSON)
            do {
        ...
        return try ProviderExecutionSupport.decodeValidated(
            ...
            validate: SummarySchemaValidator.validate
        )
    }
}
